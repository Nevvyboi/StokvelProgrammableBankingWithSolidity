...
14. **The real sandbox is stateless.** A transfer returns a payment reference and nothing changes. The money-moving demos run on a stateful mirror of the sandbox in `relayer/src/mock`; `INVESTEC_SHADOW=sandbox` fires each payout at the real sandbox as well. The speech says this out loud in DEMO 2 rather than hiding it. The live sandbox smoke run (`npm run smoke:sandbox`) is left for the owner because the build environment could not reach `openapisandbox.investec.com`.
