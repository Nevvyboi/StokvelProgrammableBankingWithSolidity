...
9. **Fonts in PowerPoint.** Neither python-pptx nor pptxgenjs can embed fonts. The decks reference Space Grotesk, Inter and JetBrains Mono by name, the TTFs and licences ship in `assets/fonts/`, and the PDF export embeds them for machines without the fonts.
10. **Design direction, from the owner mid-build: "non AI design".** The first dashboard was a grid of rounded cards with mono eyebrow labels, the generic dashboard look. It was thrown away for one idiom carried through everything: the treasurer's book. Ruled ledger rows, tabular figures, rubber stamps for state, a pencil note in the margin for whose turn it is, and the quilt as fabric rather than decoration. The slides, the HTML deck, the diagrams, the cards and the README hero all follow it.
11. **Twenty slides, not eighteen.** The run sheet's eighteen are the talk. Two appendix slides (the threat model as a table, the architecture diagram) sit after the close for questions, because the brief wants the threat model on a slide and no run-sheet slide can carry a seven-row table without breaking the 25 word rule.
12. **The hook's emoji is drawn.** The build machine had no colour emoji font, so the neutral face on the "where's December's money?" slide is three shapes, and renders the same everywhere.
13. **Visual QA needed Impress.** The image shipped with `libreoffice-core` only; `libreoffice-impress` was installed from the Ubuntu archive to export the PDF and render the slides. `slides/deck.sh` documents the dependency.
14. **The real sandbox is stateless.** A transfer returns a payment reference and nothing changes. Fixed with `relayer/src/overlay`, a transparent proxy that forwards every call to the sandbox and records each accepted transfer under the sandbox's own `PaymentReferenceNumber`, folding those into later balance, transaction and beneficiary reads. `scripts/demo.sh --sandbox` runs it on :4200 and points the relayer and the stage at it; the presenter stories become real sandbox transfers between two sandbox accounts. The offline mirror in `relayer/src/mock` stays as the default and, with `MOCK_STATELESS=true`, is how the overlay is tested (`relayer/test/overlay.test.ts`). The speech says this out loud in DEMO 2. The live sandbox run is left for the owner because the build environment could not reach `openapisandbox.investec.com`: they need real sandbox beneficiary ids in `MEMBERS_FILE` and matching `beneficiaryHash`es at deploy.
